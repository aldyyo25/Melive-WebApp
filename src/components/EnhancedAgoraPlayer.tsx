...
export const EnhancedAgoraPlayer = memo(
  ({
    ...
    const checkWebRTCSupport = () => {
      ...
      try {
        ...
        const hasWebRTC = !!(
          ...
          (window as unknown as { mozRTCPeerConnection?: unknown })
    ...
    useEffect(() => {
      ...
      const initAgora = async () => {
        try {
          ...
          if (!appId || !channel) {
            setError(
              'Missing required stream configuration (App ID or Channel)'
            );
            return;
          }
          ...
          if (!AgoraRTC.checkSystemRequirements()) {
            ...
            setError(
              'Your browser does not support video streaming. Please use a modern browser like Chrome, Firefox, Safari, or Edge.'
            );
            return;
          }
          ...
          if (!checkWebRTCSupport()) {
            ...
            setError(
              'WebRTC features are not available. Please ensure you are using HTTPS and a supported browser.'
            );
            return;
          }
          ...
          try {
            ...
            switch (error.code) {
              case 'INVALID_PARAMS':
                errorMessage =
                  'Invalid stream configuration. Please check stream parameters.';
                break;
              case 'NOT_SUPPORTED':
                errorMessage =
                  'WebRTC not supported. Please use Chrome 58+, Firefox 56+, Safari 12+, or Edge 79+.';
                break;
              case 'WEB_SECURITY_RESTRICT':
                errorMessage =
                  'Security restriction: Please use HTTPS (https://) or localhost to access this stream.';
                break;
              case 'NETWORK_ERROR':
                errorMessage =
              ...
              case 'INVALID_TOKEN':
                errorMessage =
                  'Stream authentication failed. The stream may be offline or access token expired.';
                break;
              case 'UID_CONFLICT':
                errorMessage =
                  'Connection conflict detected. Please refresh the page.';
                break;
              case 'OPERATION_ABORTED':
                errorMessage =
                  'Connection attempt was cancelled. Please try again.';
                break;
              default:
                if (error.message) {
          ...
          if (
            ...
            errorMessage =
              'Browser compatibility issue. For best experience, please use HTTPS or localhost.';
          } else if (error.message?.includes('network')) {
            errorMessage = 'Network error. Please check your connection.';
          } else if (
            error.message?.includes('security') ||
            error.message?.includes('WEB_SECURITY_RESTRICT')
          ) {
            errorMessage =
              'Security restriction: Please access this page via HTTPS or localhost.';
          }
    ...
    }, [appId, channel, token, uid, remoteUsers.length]); // Added remoteUsers.length
    ...
    if (error) {
      return (
        <div
          className={`flex items-center justify-center bg-gradient-to-br from-gray-900 to-black text-white ${className}`}
        >
        ...
        {isConnected && !hasVideo && (
          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-900 to-black text-white">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400 mt-2">
                The broadcaster will appear here shortly
              </p>
            </div>
          </div>
        )}
